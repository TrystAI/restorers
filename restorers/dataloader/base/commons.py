...
def random_unpaired_horiontal_flip(low_image):
    return tf.cond(
        tf.random.uniform(shape=(), maxval=1) < 0.5,
        lambda: low_image,
        lambda: tf.image.flip_left_right(low_image),
    )
...
def random_unpaired_vertical_flip(low_image):
    return tf.cond(
        tf.random.uniform(shape=(), maxval=1) < 0.5,
        lambda: low_image,
        lambda: tf.image.flip_up_down(low_image),
    )
